    ...
    public SudokuMatriz(int dif){
        ...
        for(int i = 0; i < size; i++){
            for(int j = 0; j < size; j+=3){
                ...
                m_principal[i][j+1] = new Position(-1);
                m_principal[i][j+2] = new Position(-1);
            }
        }
    }
...
//        return ((int)x/3)*fq+(int)y/3;
//    }
//
//    private boolean avaliarPosicao(int value){
//        if()
//    }
    ...
    private boolean avaliaQuadrante(int quadrante, int value){
        for(int i = 0; i < 3; i++){
            for(int j = 0; j < 3; j++){
                if(m_principal[i+3*((int)quadrante/fq)][j+3*(quadrante%fq)].getValue() == value){
                    return false;
                }
            }
        }
